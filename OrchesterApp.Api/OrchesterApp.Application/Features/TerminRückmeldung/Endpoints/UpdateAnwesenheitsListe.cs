...
namespace TvJahnOrchesterApp.Application.Features.TerminRückmeldung.Endpoints
{
    public static class UpdateAnwesenheitsListe
    {
        ...
        private static async Task<IResult> UpdateTerminAnwesenheitsListe(
            [FromBody] UpdateAnwesenheitsCommand updateAnwesenheitsCommand, ISender sender,
        ...
        private record UpdateAnwesenheitsCommand(Guid TerminId, AnwesenheitResponseEntry[] AnwesenheitResponseEntries)
            : IRequest<Unit>;

        private record AnwesenheitResponseEntry(
        ...
        private class UpdateAnwesenheitsCommandHandler : IRequestHandler<UpdateAnwesenheitsCommand, Unit>
        {
            ...
            public async Task<Unit> Handle(UpdateAnwesenheitsCommand request, CancellationToken cancellationToken)
            {
                ...
                {
                    termin.AnwesenheitZuTermin(OrchesterMitgliedsId.Create(anwesenheit.OrchesterMitgliedsId),
                        anwesenheit.IstAnwesend, anwesenheit.KommentarAnwesenheit);
                }
