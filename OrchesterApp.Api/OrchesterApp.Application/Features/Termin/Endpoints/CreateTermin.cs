...
namespace TvJahnOrchesterApp.Application.Features.Termin.Endpoints
{
    public static class TerminCreate
    {
        ...
        private class CreateTerminCommandHandler : IRequestHandler<CreateTerminCommand, OrchesterApp.Domain.TerminAggregate.Termin>
        {
            ...
            public async Task<OrchesterApp.Domain.TerminAggregate.Termin> Handle(CreateTerminCommand request, CancellationToken cancellationToken)
            {
                ...
                var termin = OrchesterApp.Domain.TerminAggregate.Termin.Create(terminRückmeldungOrchesterMitglieder, request.Name, request.TerminArt, request.StartZeit, request.EndZeit, treffpunkt, request.Noten?.ToList(), request.Uniform?.ToList(), zusätzlicheInfo: request.WeitereInformationen, image: compressedImage);
