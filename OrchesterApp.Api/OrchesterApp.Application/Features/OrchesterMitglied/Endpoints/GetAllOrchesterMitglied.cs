...
namespace TvJahnOrchesterApp.Application.Features.OrchesterMitglied.Endpoints
{
    public static class GetAllOrchesterMitglied
    {
        ...
        private static async Task<IResult> GetAllOrchesterMitglieder(ISender sender,
        ...
        private record GetAllOrchesterMitgliederResponse(
            Guid Id,
            string Vorname,
            string Nachname,
            string? Image,
        ...
        private class GetAllOrchesterMitgliederQueryHandler : IRequestHandler<GetAllOrchesterMitgliederQuery,
            GetAllOrchesterMitgliederResponse[]>
        {
            ...
            public async Task<GetAllOrchesterMitgliederResponse[]> Handle(GetAllOrchesterMitgliederQuery request,
                CancellationToken cancellationToken)
            {
                ...
                var activeMitglieder = orchesterMitglieder.Where(m => m.IsAktiv());

                foreach (var mitglied in activeMitglieder)
                {
                    Instrument? instrument = null;
                    if (mitglied.DefaultInstrument is not null)
                    {
                        instrument =
                            await instrumentRepository.GetByIdAsync((int)mitglied.DefaultInstrument, cancellationToken);
                    }

                    var imageAsBase64 = TransformImageService.ConvertByteArrayToBase64(mitglied.Image);

                    result.Add(new GetAllOrchesterMitgliederResponse(mitglied.Id.Value, mitglied.Vorname,
                        mitglied.Nachname, imageAsBase64, instrument?.Value, mitglied.MemberSinceInYears));
                }
