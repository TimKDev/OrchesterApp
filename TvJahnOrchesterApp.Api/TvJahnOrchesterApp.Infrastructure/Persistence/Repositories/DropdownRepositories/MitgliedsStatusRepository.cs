...
{
    internal class MitgliedsStatusRepository : IDropdownRepository
    {
        private readonly OrchesterDbContext _context;

        public MitgliedsStatusRepository(OrchesterDbContext context)
        {
            _context = context;
        }

        public DropdownNames DropdownName => DropdownNames.MitgliedsStatus;

        public Task<DropdownItem[]> GetAllDropdownItemsAsync(CancellationToken cancellationToken)
        {
            return _context.Set<MitgliedsStatus>().Select(x => new DropdownItem(x.Id, x.Value)).ToArrayAsync(cancellationToken);
        }
    }
}
